...
/**
 * Adapt Perch's compact wire protocol to the same Next.js App Route function
...
function executeNextRoute(
  ...
  // through Perch's synchronous ABI after the framework work has completed.
  GET(nextRequest);
...
/** Required compact PCH2 application-library ABI. */
export function handle(frame: Buffer): Buffer {
  if (
    ...
    throw new Error("invalid Perch HTTP frame");
  }
