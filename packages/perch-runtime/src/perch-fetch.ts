// @perch/runtime — Outbound HTTP with retries and allowlist.
//
// Wraps Perry's fetch stdlib with retry logic, timeout enforcement,
// and domain allowlist checking. The allowlist is read from
// PERCH_FETCH_ALLOWLIST (comma-separated, set by perch-worker from the
// deployment's perch.toml [capabilities.fetch.allowlist]).
//
// Usage:
//   import { perchFetch } from "@perch/runtime";
//   const resp = await perchFetch("https://api.stripe.com/v1/charges", {
//     method: "POST",
//     headers: { Authorization: "Bearer " + token },
//     body: JSON.stringify(payload),
//   });

const ALLOWLIST_RAW = process.env.PERCH_FETCH_ALLOWLIST || "";
...
export async function perchFetch(
  url: string,
  init?: {
    method?: string;
  ...
  if (!isDomainAllowed(url)) {
    throw new Error(
      ...
      "Add the domain to perch.toml [capabilities.fetch.allowlist]."
    );
  }
