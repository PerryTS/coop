// @coop/runtime — Object storage.
//
// Files on disk in v0, wrapped in an S3-ish interface. coop-worker sets
// COOP_STORAGE_DIR to a per-deployment directory; all paths are relative
// to that root.
//
// Usage:
...
// ──────────────────────────────────────────────────────────────────────
//
//   <COOP_STORAGE_DIR>/          one directory per deployment, created and
//                                 owned by coop-worker, never by this module
...
const STORAGE_DIR = process.env.COOP_STORAGE_DIR || "";
...
function checkConfigured(): void {
  if (STORAGE_DIR === "") {
    throw new Error(
      "storage is not available: COOP_STORAGE_DIR is not set. The object " +
      ...
      "coop.toml, and configure [paths] storage_dir in runtime.toml."
    );
  }
}
