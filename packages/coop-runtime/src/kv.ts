// @coop/runtime — Key-value store (Redis).
//
// Wraps Perry's ioredis stdlib. coop-worker sets COOP_REDIS_URL and
// COOP_REDIS_PREFIX env vars; the prefix is prepended to every key so
...
//     defaulting to ON. Passing COOP_REDIS_URL here would do nothing.
//     coop-worker translates COOP_REDIS_URL into those four variables before
//     loading this library (crates/coop-worker/src/deployment_env.rs); this
//     module treats COOP_REDIS_URL purely as the "is kv configured?" flag.

import Redis from "ioredis";

// Read at module-init time, which Perry runs inside perry_module_init() while
// coop-worker's DeploymentHost::load is still on the stack — after the worker
// has exported the environment.
const PREFIX = process.env.COOP_REDIS_PREFIX || "";
const REDIS_URL = process.env.COOP_REDIS_URL || "";
...
function checkConfigured(): void {
  if (PREFIX === "") {
    throw new Error(
      "kv is not available: COOP_REDIS_PREFIX is not set. Without a " +
      ...
      "process — set isolation.class = \"dedicated\" in coop.toml."
    );
  }
  if (REDIS_URL === "") {
    throw new Error(
      "kv is not configured: COOP_REDIS_URL is not set. Configure Redis in " +
      "runtime.toml [redis] url so coop-worker can export it to this " +
      "deployment."
    );
  }
}
