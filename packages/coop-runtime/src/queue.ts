// @coop/runtime — host-owned durable queue enqueue.
//
// Usage:
...
declare function js_coop_queue_enqueue(
  queueName: string,
  payloadJson: string,
  delayMs: number,
): number;

declare function js_coop_queue_enqueue_raw(
  queueName: string,
  payload: Buffer,
  delayMs: number,
): number;
...
function checkResult(result: number): void {
  if (result !== 0) {
    throw new Error("Coop durable queue enqueue failed with code " + result);
  }
}
...
export const queue = {
  async send(queueName: string, payload: any, opts?: QueueSendOptions): Promise<void> {
    ...
    checkResult(js_coop_queue_enqueue(queueName, encoded, queueDelay(opts)));
  },
  ...
  async sendRaw(queueName: string, payload: Buffer, opts?: QueueSendOptions): Promise<void> {
    ...
    checkResult(js_coop_queue_enqueue_raw(queueName, payload, queueDelay(opts)));
  },
};
