...
Two protocols, both defined in `crates/perch-host-abi` so the daemon, the worker,
...
| `APP_LIBRARY_BOUNDARY_VERSION` | Perch's cached native-boundary audit; bumping it forces existing images back through the audit before their cached result is trusted |
...
## Writing a `PCH2` handler directly

Most handlers should use [`@perch/runtime`](runtime-api.md), which hides this.
