// Checkpoint 1 smoke test — minimal Coop deployment plugin.
//
...
// wire protocol in coop-host-abi (one "route" tool per deployment,
// JSON-over-string).
//
// Used by crates/coop-worker/tests/ to verify the full dispatch path:
//   coop-worker process
//     → dlopens echo.dylib
...
export function activate(api: PluginApi): void {
  ...
  api.registerTool("route", "Echo HTTP router", (argsJson: any) => {
    ...
    // coop-worker called perry_plugin_invoke_tool with a string arg.
    ...
    // worker code, so we implement it inline with Buffer. (@coop/runtime
