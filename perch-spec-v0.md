...
- **Operational simplicity** — one binary, one config file, `systemctl restart perch`
...
- Bugs in LLVM or the Perch runtime itself
...
- **Runtime bugs**: a bug in the Perch binary could be exploitable. Mitigation: write the runtime in Rust, keep the host function surface small, fuzz the boundary.
...
Perch has three tiers of execution state, each with different lifetimes and isolation properties. Understanding these is critical because they determine the entire runtime shape.

### Tier 1: The daemon (`perch`)

One long-running process per box. Runs as a systemd service. Responsibilities:

- Watches the `deployments/` directory for changes
- Spawns and supervises `perch-worker` processes (one per deployment)
...
- Exposes the admin UI at `/_perch/admin`
...
### Tier 2: Worker processes (`perch-worker`)
...
- A symbol table exporting each entry point under a known name (`__perch_handler_ingest`, `__perch_cron_daily_digest`, etc.)
- Undefined symbols for host functions (`__perch_host_fetch`, `__perch_host_db_query`, etc.), resolved by the runtime at `dlopen` time
...
- `@perch/runtime` — the host function namespace (database, kv, fetch, log, etc.)
...
HTTP handler:
```typescript
import type { Request, Response } from "@perch/runtime";
...
Cron job:
```typescript
import type { CronContext } from "@perch/runtime";
...
Queue worker:
```typescript
import type { QueueMessage } from "@perch/runtime";
...
Workers come in exactly four shapes. If it doesn't fit one of these, it doesn't belong in Perch.
...
```typescript
import { db, log } from "@perch/runtime";
...
```typescript
import { db, fetch, log } from "@perch/runtime";
...
```typescript
import { fetch, log } from "@perch/runtime";
...
Perch includes a set of built-in data services. Each is implemented as a shared instance that the runtime manages, with per-deployment namespacing enforced at the host function level.
...
```typescript
import { fetch } from "@perch/runtime";
...
This is the complete list of what workers can see. Every capability in Perch is one of these; nothing else exists from the worker's perspective.
...
/var/lib/perch/
  ...
  secrets/
    ...
    gscmaster.enc
  logs/
    perch.sqlite               # structured log store
...
Per-deployment config in `perch.toml`:
...
Runtime config in `/var/lib/perch/runtime.toml`:

```toml
[http]
listen = "0.0.0.0:80"
tls_listen = "0.0.0.0:443"
tls_cert = "/etc/letsencrypt/live/perch.dev/fullchain.pem"
tls_key = "/etc/letsencrypt/live/perch.dev/privkey.pem"

[postgres]
url = "postgres://perch:xxx@localhost/perch"
...
Every invocation produces a log record with timestamp, deployment, worker, trace ID, duration, status, and any fields the worker added via `log.info()`. Stored in a SQLite database under `/var/lib/perch/logs/` (yes, SQLite here — logs are append-only time-series and SQLite handles them beautifully, and we explicitly don't want log storage hitting the same Postgres as application data).
...
Built-in metrics exposed at `/_perch/metrics` (Prometheus format) and rendered on the admin dashboard:
...
Server-rendered HTML at `/_perch/admin`, protected by a single password. Pages:
...
```bash
# Push to the box
rsync -av ./chirp/ root@box:/var/lib/perch/deployments/chirp/

# The daemon notices the change (inotify), recompiles, spawns a new
# perch-worker process for chirp, drains the old one, kills it.
# Total downtime: ~1 second.

# Or, if you want to trigger manually:
ssh root@box "perch reload chirp"
```

### Rollback

Previous versions are kept in `/var/lib/perch/compiled/` for N generations (configurable). Rollback is:

```bash
perch rollback chirp
```
...
3. **One `perch-worker` process per user (or per group of related users).** The existing per-deployment process model already handles this — just group deployments by user and the daemon spawns workers per user-group rather than per-deployment-within-a-single-user. ~2 hours of daemon logic.
...
5. **TLS with SNI routing.** A subdomain per user (`username.perch.dev`), ACME for certs. ~a day.
...
Run on a bigger box. A 16-core, 64GB Hetzner dedicated machine at ~€50/month can run hundreds of Perch deployments comfortably because per-deployment idle overhead is single-digit MB. For the entire Skelpo portfolio, this is the whole answer.
...
- **Horizontal scaling of a single deployment across multiple boxes.** If one deployment needs more than one box, it's outgrown Perch. Go run it on real infrastructure.
- **Multi-region.** One box is in one region. If you need multi-region, run multiple independent Perch instances.
...
**Exit criterion**: `echo 'export default () => log.info("hi")' | perry worker-compile -o hello.so && perch run hello.so` works.
...
- Typed query builder compiled into `@perch/runtime`
...
- `perch` CLI: `dev`, `deploy`, `rollback`, `logs`, `reload`, `status`
- Documentation for writing the first deployment

**Exit criterion**: Chirp is running on Perch in production on a Hetzner box, you can see its metrics in the admin UI, and you've turned off whatever was hosting it before.
