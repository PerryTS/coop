...
//!
//! **Host ↔ deployment dylib.** Applications exchange a compact `PCH2`
...
pub const APP_LIBRARY_ABI_VERSION: u32 = 2;

/// Version of Perch's cached native-library boundary audit. This is separate
...
/// Perch refuses to load a descriptor built by another Perry version.
...
pub struct AppLibraryManifest {
    ...
    /// Digest of Perch's semantic Perry argv, explicitly propagated build
...
impl AppLibraryManifest {
    pub fn adjacent_path(library: &Path) -> PathBuf {
        library.with_extension("perch-lib.json")
    }
...
const APP_FRAME_MAGIC: &[u8; 4] = b"PCH2";
