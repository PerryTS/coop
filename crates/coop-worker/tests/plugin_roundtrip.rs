...
use coop_app_host::initialize_runtime_libraries;
...
fn provider_paths(workspace: &std::path::Path) -> (PathBuf, PathBuf) {
    ...
    (
        std::env::var_os("COOP_TEST_RUNTIME_LIBRARY")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
                workspace.join(format!("var/coop/lib/libperry_runtime.{extension}"))
            }),
        std::env::var_os("COOP_TEST_STDLIB_LIBRARY")
            .map(PathBuf::from)
            .unwrap_or_else(|| workspace.join(format!("var/coop/lib/libperry_stdlib.{extension}"))),
    )
}

fn source_app_path(workspace: &std::path::Path) -> PathBuf {
    if let Some(path) = std::env::var_os("COOP_TEST_APP_LIBRARY") {
...
async fn host_buffer_churn_is_reclaimed_by_perry() {
    ...
    let host = DeploymentHost::load_with_options(
        ...
        &app,
        None,
        coop_app_host::host::DeploymentHostOptions {
...
async fn repeated_load_dispatch_shutdown_reclaims_executor_threads() {
    ...
    initialize_runtime_libraries(&runtime, &stdlib).expect("load Perry provider pair");
    let cycles = std::env::var("COOP_LIFECYCLE_CYCLES")
        .ok()
    ...
    let baseline_rss = coop_app_host::process_rss_kib();
    ...
    for iteration in 0..cycles {
        ...
        if (iteration + 1) % 10 == 0 || iteration + 1 == cycles {
            ...
            if let Some(rss) = coop_app_host::process_rss_kib() {
    ...
    let final_rss = coop_app_host::process_rss_kib();
    eprintln!(
    ...
    for index in 0..100 {
        ...
        if manifest.boundary_verified {
            ...
            manifest.library_sha256 = Some(
                coop_app_host::plugin_host::library_sha256(&app).expect("hash copied app image"),
            );
        }
