//! coop-worker — per-deployment host process.
//!
//! Spawned by coop-daemon with `--deployment <name>`. Responsibilities:
//!
...
//!
//! 3. Listen on `/var/lib/coop/sockets/<deployment>.sock` for framed
//!    requests from coop-daemon. Each message is length-prefixed JSON
//!    (see coop_host_abi).
//!
...
use coop_app_host::{host, initialize_runtime_libraries_with_verification, ProviderVerification};
...
struct Cli {
    ...
    /// /var/lib/coop/sockets.
    #[arg(long, default_value = "/var/lib/coop/sockets")]
    sockets_dir: PathBuf,
    ...
    #[arg(long, env = "COOP_PERRY_RUNTIME_LIBRARY")]
    perry_runtime: PathBuf,

    /// Separately packaged Perry stdlib shared library.
    #[arg(long, env = "COOP_PERRY_STDLIB_LIBRARY")]
    ...
    /// arguments. Absent leaves `@coop/runtime`'s `kv` unconfigured, and `kv`
    /// then throws on use rather than reaching a default server.
    #[arg(long, env = "COOP_REDIS_URL")]
    ...
    /// exports it as `COOP_STORAGE_DIR`; the application never sees the root.
    #[arg(long, env = "COOP_STORAGE_ROOT")]
    ...
    #[arg(long, env = "COOP_QUEUE_POSTGRES_URL")]
    queue_postgres_url: Option<String>,

    #[arg(long, env = "COOP_QUEUE_POSTGRES_MAX_CONNECTIONS", default_value_t = 2)]
    queue_postgres_max_connections: u32,

    #[arg(long, env = "COOP_QUEUE_POLICIES_JSON")]
...
fn main() -> anyhow::Result<()> {
    ...
    info!(
        ...
        "coop-worker starting"
    );

    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .thread_name(format!("coop-worker-{worker_label}"))
        .build()?;

    runtime.block_on(async move {
        if let Some(url) = cli.queue_postgres_url.as_deref() {
            let store = Arc::new(
                coop_app_host::queue_store::QueueStore::connect(
                    url,
                    cli.queue_postgres_max_connections,
                )
                .await?,
            );
            coop_app_host::queue_store::initialize_queue_gateway(
                store,
        ...
        if cli.deployment_context_id != 0 {
            if cli.queue_postgres_url.is_none() {
                return Err(anyhow::anyhow!(
                    "deployment context requires COOP_QUEUE_POSTGRES_URL"
                ));
            }
            let raw_policies = cli.queue_policies_json.as_deref().ok_or_else(|| {
                anyhow::anyhow!("deployment context requires COOP_QUEUE_POLICIES_JSON")
            })?;
            ...
            coop_app_host::queue_store::register_enqueue_context(
                ...
                policies
                    ...
                    .map(|queue| {
                        (
                            queue.name,
                            coop_app_host::queue_store::EnqueuePolicy {
        ...
        // `@coop/runtime`'s kv and storage capture their configuration in
        ...
        let exported = coop_app_host::export_deployment_environment(
            deployment,
            &coop_app_host::DeploymentServices {
        ...
        coop_app_host::queue_store::unregister_enqueue_context(cli.deployment_context_id);
        result
    })?;
    ...
    fn capability_configuration_is_reachable_by_flag_and_by_environment() {
        ...
        assert_eq!(
            Cli::command()
                ...
                .filter(|env| *env == "COOP_REDIS_URL" || *env == "COOP_STORAGE_ROOT")
                .count(),
            2,
            "the daemon sets COOP_REDIS_URL and COOP_STORAGE_ROOT; both must \
