...
//! pointers. Consequently neither `coop` nor `coop-worker` statically embeds
//! Perry.
...
/// Perry's FFI root scope. A host that holds a JS value across anything that
/// can run JS must root it: an evacuating minor MOVES the object, and a cached
/// raw address becomes a stale from-space pointer.
pub(crate) type JsFfiRootScopeEnter = unsafe extern "C" fn() -> usize;
...
pub type CoopQueueEnqueueCallback =
    unsafe extern "C" fn(u64, *const u8, usize, *const u8, usize, u64) -> i32;
pub(crate) type CoopRegisterQueueEnqueueCallback =
    unsafe extern "C" fn(CoopQueueEnqueueCallback) -> i32;
pub(crate) type CoopSetDeploymentContext = unsafe extern "C" fn(u64);
pub(crate) type CoopClearDeploymentContext = unsafe extern "C" fn();
...
pub(crate) struct RuntimeApi {
    ...
    pub js_ffi_root_scope_enter: JsFfiRootScopeEnter,
    ...
    pub coop_host_register_queue_enqueue_callback: CoopRegisterQueueEnqueueCallback,
    pub coop_host_set_deployment_context: CoopSetDeploymentContext,
    pub coop_host_clear_deployment_context: CoopClearDeploymentContext,
}
...
/// a root-owned, non-writable filesystem boundary and Coop runs unprivileged.
    ...
    let loaded = unsafe {
        (|| -> Result<RuntimeLibraries> {
            let api = RuntimeApi {
                ...
                js_ffi_root_scope_enter: load_symbol(runtime_handle, "js_ffi_root_scope_enter")?,
                ...
                coop_host_register_queue_enqueue_callback: load_symbol(
                    stdlib_handle,
                    "coop_host_register_queue_enqueue_callback",
                )?,
                coop_host_set_deployment_context: load_symbol(
                    stdlib_handle,
                    "coop_host_set_deployment_context",
                )?,
                coop_host_clear_deployment_context: load_symbol(
                    stdlib_handle,
                    "coop_host_clear_deployment_context",
                )?,
            };
            ...
            let probe: unsafe extern "C" fn() -> usize =
                load_symbol(stdlib_handle, "coop_stdlib_runtime_probe")?;
...
fn validate_root_owned_immutable_package(
    ...
    if unsafe { libc::geteuid() } == 0 {
        return Err(anyhow!(
            "root_owned_immutable provider verification requires an unprivileged Coop process"
        ));
    }
...
fn reject_effectively_writable(path: &Path, kind: &str) -> Result<()> {
    ...
    if unsafe { libc::access(path_c.as_ptr(), libc::W_OK) } == 0 {
        Err(anyhow!(
            "root_owned_immutable Perry {kind} is writable by the Coop service identity: {}",
            path.display()
        ))
    } else {
        Ok(())
...
pub fn register_queue_enqueue_callback(callback: CoopQueueEnqueueCallback) -> Result<()> {
    let registered = unsafe { (runtime_api().coop_host_register_queue_enqueue_callback)(callback) };
    if registered == 1 {
        Ok(())
    } else {
        Err(anyhow!(
            "Perry stdlib rejected the Coop queue enqueue callback"
        ))
    }
}

pub(crate) fn set_deployment_context(deployment_id: u64) {
    unsafe { (runtime_api().coop_host_set_deployment_context)(deployment_id) }
}

pub(crate) fn clear_deployment_context() {
    unsafe { (runtime_api().coop_host_clear_deployment_context)() }
}
