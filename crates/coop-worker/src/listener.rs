...
//!
//! coop-daemon connects to our per-deployment socket at
...
//! ```
//!
//! Max frame size is `coop_host_abi::MAX_FRAME_SIZE`. Frames larger than
    ...
    match req {
        WorkerRequest::Hello(ClientHello {
            ...
            let hello = WorkerResponse::Hello {
                abi_version: ABI_VERSION,
                worker_name: format!("coop-worker/{}", env!("CARGO_PKG_VERSION")),
                deployment: deployment.to_string(),
            };
    ...
    match req {
        ...
        WorkerRequest::Dispatch {
            ...
            Err(e) => {
                ...
                WorkerResponse::DispatchResult {
                    request_id,
                    response: coop_host_abi::DeploymentResponse {
                        status: 500,
                        headers: Default::default(),
                        body_base64: base64_encode(
                            format!("coop: dispatch error: {}", e).as_bytes(),
                        ),
                    },
                }
            }
        ...
        WorkerRequest::Queue {
            ...
            Err(e) => WorkerResponse::QueueResult {
                request_id,
                disposition: coop_host_abi::QueueDisposition::Nack,
                error: Some(e.to_string()),
            },
        },
