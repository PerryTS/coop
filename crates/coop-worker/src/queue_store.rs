...
//!
//! Queue rows are global to one Coop installation and carry deployment
...
CREATE TABLE IF NOT EXISTS coop_queue_schema_migrations (
    version INTEGER PRIMARY KEY,
    installed_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp()
);

CREATE TABLE IF NOT EXISTS coop_queue_messages (
    id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS coop_queue_messages_claim_idx
    ON coop_queue_messages
        (deployment, queue_name, available_at, lease_expires_at, created_at, id);

CREATE TABLE IF NOT EXISTS coop_queue_dead_letters (
    id TEXT PRIMARY KEY,
    deployment TEXT NOT NULL,
    queue_name TEXT NOT NULL,
    payload BYTEA NOT NULL,
...
CREATE INDEX IF NOT EXISTS coop_queue_dead_letters_lookup_idx
    ON coop_queue_dead_letters (deployment, queue_name, failed_at, id);

INSERT INTO coop_queue_schema_migrations(version)
VALUES (1)
...
UPDATE coop_queue_messages AS message
SET lease_owner = $3,
    lease_token = $4,
...
WITH removed AS (
    DELETE FROM coop_queue_messages
...
/// Install the process-wide callback used by `@coop/runtime`. The callback is
...
impl QueueStore {
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        let version = client
            .query_opt(
                "SELECT to_regclass('coop_queue_schema_migrations') IS NOT NULL",
                &[],
            )
            .await
        ...
        if version {
            let latest: Option<i32> = client
                .query_one("SELECT max(version) FROM coop_queue_schema_migrations", &[])
                .await
    ...
    pub async fn ack(&self, message: &ClaimedMessage) -> Result<bool> {
        ...
        let removed = client
            .execute(
                "DELETE FROM coop_queue_messages WHERE id = $1 AND lease_token = $2 AND attempts = $3",
    ...
    pub async fn stats(&self, deployment: &str, queue_name: &str) -> Result<QueueStats> {
        ...
        let dead_letters: i64 = client
            .query_one(
                "SELECT count(*)::bigint FROM coop_queue_dead_letters WHERE deployment = $1 AND queue_name = $2",
                &[&deployment, &queue_name],
            )
            .await
    ...
    pub async fn prune_dead_letters(
        ...
        client
            .execute(
                r#"
                DELETE FROM coop_queue_dead_letters
                WHERE deployment = $1 AND queue_name = $2
...
mod tests {
    ...
    fn schema_and_claim_are_explicit_and_transactional() {
        ...
        assert!(MIGRATION_SQL.contains("coop_queue_dead_letters"));
    ...
    #[tokio::test]
    #[ignore = "requires COOP_TEST_POSTGRES_URL"]
    async fn postgres_proves_leases_retries_dlq_and_binary_payloads() {
        let url = std::env::var("COOP_TEST_POSTGRES_URL")
            .expect("COOP_TEST_POSTGRES_URL must point to a disposable test database");
        ...
        client
            .execute(
                "DELETE FROM coop_queue_messages WHERE deployment = $1",
                &[&deployment],
            )
            .await
            .unwrap();
        client
            .execute(
                "DELETE FROM coop_queue_dead_letters WHERE deployment = $1",
                &[&deployment],
            )
            .await
            .unwrap();
