...
//!
//! `@perch/runtime`'s `kv` and `storage` modules read their configuration from
...
//!
//! 1. **The key prefix must be injective.** `kv` prepends `PERCH_REDIS_PREFIX`
...
//!    worker translates the operator's `PERCH_REDIS_URL` into those four
...
/// `packages/perch-runtime/src/kv.ts`.
pub const REDIS_PREFIX_VAR: &str = "PERCH_REDIS_PREFIX";
...
pub const REDIS_URL_VAR: &str = "PERCH_REDIS_URL";
/// Per-deployment object-storage root. Read by
/// `packages/perch-runtime/src/storage.ts`.
pub const STORAGE_DIR_VAR: &str = "PERCH_STORAGE_DIR";
...
pub fn redis_key_prefix(deployment: &str) -> String {
    format!("perch{KV_PREFIX_SEPARATOR}{deployment}{KV_PREFIX_SEPARATOR}")
}
...
mod tests {
    ...
    fn prefix_is_always_exported_even_without_a_configured_redis() {
        // kv.ts reads PERCH_REDIS_PREFIX at module init unconditionally. If it
        ...
        assert_eq!(lookup(&vars, REDIS_PREFIX_VAR), Some("perch:chirp:"));
    ...
    fn two_deployments_cannot_reach_each_others_keys() {
        ...
        assert_eq!(a.clone() + "b:session", "perch:a:b:session");
        assert!(
    ...
    fn storage_dir_is_scoped_to_the_deployment_and_created_on_export() {
        ...
        assert_eq!(std::env::var(REDIS_PREFIX_VAR).unwrap(), "perch:chirp:");
    }
