...
//! Next App Route executes through a Perch-compiled application library on the
...
//! fixture through Perch's own compile pipeline; anything that stops the
//! rebuild fails the test with the reason.

use perch_app_host::host::DeploymentHost;
use perch_app_host::{initialize_runtime_libraries, perry_provider_identity};
...
fn manifest_rejection(manifest: &AppLibraryManifest) -> Option<String> {
    ...
    if manifest.perry_version != perch_app_host::PERRY_RUNTIME_VERSION {
        return Some(format!(
            "Perry version {} != host {}",
            manifest.perry_version,
            perch_app_host::PERRY_RUNTIME_VERSION
        ));
    }
    if manifest.perry_commit != perch_app_host::PERRY_RUNTIME_COMMIT {
        return Some(format!(
            "Perry commit {} != host {}",
            manifest.perry_commit,
            perch_app_host::PERRY_RUNTIME_COMMIT
        ));
    }
...
async fn next_fixture_uses_required_http_abi() {
    ...
    let extension = library_extension();
    let runtime = workspace.join(format!("var/perch/lib/libperry_runtime.{extension}"));
    let stdlib = workspace.join(format!("var/perch/lib/libperry_stdlib.{extension}"));
    assert!(
    ...
    let namespace = workspace.join("target/next-benchmark/perch-run/compiled/next-bench");
    ...
    if app.is_none() {
        ...
        eprintln!(
            "rebuilding the Next fixture through Perch ({}); rejected candidates: {rejected:?}",
            script.display()
        );
    ...
    // The loaded bytes must be the ones Perch published: `DeploymentHost::load`
