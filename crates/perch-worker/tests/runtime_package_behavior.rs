//! Behavioural tests for `@perch/runtime`'s `kv` and `storage`.
//!
...
fn runtime_package_src() -> PathBuf {
    workspace_root().join("packages/perch-runtime/src")
}
...
impl Fixture {
    fn new() -> Self {
        ...
        std::fs::write(
            ...
            r#"{"name":"perch-runtime-behavior","private":true,"type":"module"}"#,
        )
        .unwrap();
    ...
    fn run(&self, script: &str, env: &[(&str, &str)]) -> Output {
        ...
        // pass by accident.
        command.env_remove("PERCH_REDIS_PREFIX");
        command.env_remove("PERCH_REDIS_URL");
        command.env_remove("PERCH_STORAGE_DIR");
...
fn node_binary() -> &'static str {
    ...
    ONCE.get_or_init(|| {
        let version = Command::new("node").arg("--version").output().expect(
            "node is required to test @perch/runtime's TypeScript modules; \
...
for (const call of CALLS) {
  assert.ok(call[1].startsWith("perch:chirp:"),
    `unprefixed key reached redis: ${JSON.stringify(call)}`);
}
assert.deepEqual(CALLS[0], ["set", "perch:chirp:session:abc", "payload"]);
console.log("OK");
"#,
        &[
            ("PERCH_REDIS_PREFIX", "perch:chirp:"),
            ("PERCH_REDIS_URL", "redis://127.0.0.1:6379"),
        ],
    );
...
assert.deepEqual(CALLS, [
  ["set", "perch:chirp:session:abc", "payload"],
  ["expire", "perch:chirp:session:abc", 3600],
]);
assert.equal(TTL.get("perch:chirp:session:abc"), 3600,
...
for (const op of [() => kv.get("k"), () => kv.set("k", "v"),
                  () => kv.del("k"), () => kv.incr("k")]) {
  await assert.rejects(op, /PERCH_REDIS_PREFIX is not set/);
}
...
import assert from "node:assert/strict";
const { kv } = await import("./kv.ts");
await assert.rejects(() => kv.get("k"), /PERCH_REDIS_URL is not set/);
console.log("OK");
"#,
        &[("PERCH_REDIS_PREFIX", "perch:chirp:")],
    );
...
console.log("OK");
"#,
        &[("PERCH_STORAGE_DIR", root.to_str().unwrap())],
    );
...
for (const op of [() => storage.put("k", "v"), () => storage.get("k"),
  ...
  await assert.rejects(op, /PERCH_STORAGE_DIR is not set/);
}
