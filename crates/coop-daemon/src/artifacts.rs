...
pub const PACKAGE_CONFIG_FILE: &str = "deployment.coop.json";
pub const STATIC_MANIFEST_FILE: &str = "static.coop-manifest.json";
const STATIC_ROOT: &str = ".coop-static";
const STATE_FILE: &str = ".coop-deployment-state.json";
const STATE_TEMP_PREFIX: &str = ".coop-deployment-state.tmp-";
const STATE_VERSION: u32 = 1;
const PACKAGE_DIGEST_DOMAIN: &[u8] = b"coop-application-package-v1\0";
...
fn validate_staging_tree(path: &Path) -> Result<()> {
        ...
        .flatten()
    {
        ...
        if file_type.is_dir() && name == STATIC_ROOT {
            validate_plain_tree(&entry.path())?;
        } else if file_type.is_dir() && name == ".coop-source" {
...
fn test_pause_at_crash_point(point: &str) {
    if !std::env::var("COOP_TEST_ARTIFACT_CRASH_POINT").is_ok_and(|value| value == point) {
        return;
    }
    let marker = std::env::var_os("COOP_TEST_ARTIFACT_CRASH_MARKER")
...
mod tests {
    ...
    fn publish_fixture(store: &ArtifactStore, deployment: &str, marker: &str) -> VerifiedPackage {
        ...
        let manifest = AppLibraryManifest {
            abi_version: coop_host_abi::APP_LIBRARY_ABI_VERSION,
            ...
            handler_abi: coop_host_abi::HandlerAbi::Wrapped,
            cron_entries: vec![],
            queue_entries: vec![],
    ...
    fn static_assets_are_snapshotted_rewritten_and_integrity_checked() {
        ...
        assert_eq!(
            std::fs::read(package_dir.join(".coop-static/0/index.html")).unwrap(),
            b"version one"
        );
        std::fs::write(package_dir.join(".coop-static/0/index.html"), b"tampered").unwrap();
    ...
    fn os_process_kill_matrix_recovers_durable_boundaries() {
        const CHILD_ENV: &str = "COOP_TEST_ARTIFACT_CRASH_CHILD";
        const ROOT_ENV: &str = "COOP_TEST_ARTIFACT_CRASH_ROOT";
        const PACKAGE_ENV: &str = "COOP_TEST_ARTIFACT_CRASH_PACKAGE";
        const TEST_NAME: &str =
        ...
        for point in [
            ...
            command
                ...
                .env(CHILD_ENV, point)
                .env(ROOT_ENV, &root)
                .env("COOP_TEST_ARTIFACT_CRASH_POINT", point)
                .env("COOP_TEST_ARTIFACT_CRASH_MARKER", &marker)
                .stdout(Stdio::null())
                .stderr(Stdio::null());
