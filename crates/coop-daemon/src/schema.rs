...
//! `_coop_migrations` table inside the deployment's schema.
//!
...
/// 2. `CREATE TABLE IF NOT EXISTS deployment_<name>._coop_migrations (...)`
/// 3. Read `<deployment_dir>/migrations/*.sql`, sort by filename
/// 4. For each migration not yet in `_coop_migrations`: execute and record
///
...
pub async fn provision_and_migrate(
    ...
    //   3. sqlx::query("CREATE TABLE IF NOT EXISTS ..._coop_migrations (
    //        name TEXT PRIMARY KEY, applied_at TIMESTAMPTZ DEFAULT now()
    //      )").execute(&pool)
    //   4. For each file: check if name is in _coop_migrations, if not
    //      execute the SQL and INSERT into _coop_migrations
