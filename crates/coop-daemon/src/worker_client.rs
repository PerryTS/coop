//! Daemon-side client for the coop-worker Unix-socket protocol.
//!
//! A `WorkerClient` owns a connection to a single running coop-worker
...
/// One connection to one coop-worker process.
...
impl WorkerClient {
    ...
    async fn connect_inner(
        ...
        let hello = WorkerRequest::Hello(ClientHello {
            ...
            client_name: format!("coop-daemon/{}", env!("CARGO_PKG_VERSION")),
        });
    ...
    pub async fn fire_cron(&self, context: coop_host_abi::CronContext) -> Result<()> {
        let request_id = self.next_request_id.fetch_add(1, Ordering::Relaxed);
    ...
    pub async fn deliver_queue(
        &self,
        message: coop_host_abi::QueueMessage,
    ...
    timeout(SHARD_CONTROL_TIMEOUT, async {
        ...
        let hello_sent = write_frame(
            ...
            &WorkerRequest::Hello(ClientHello {
                abi_version: ABI_VERSION,
                client_name: format!("coop-daemon-control/{}", env!("CARGO_PKG_VERSION")),
            }),
        )
        .await?;
...
mod tests {
    use super::*;
    use coop_host_abi::WorkerQueuePolicy;
