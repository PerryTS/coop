//! TOML config shapes for `runtime.toml` (box-wide) and `coop.toml`
//! (per-deployment).
//!
...
/// Top-level box configuration. Lives at `/var/lib/coop/runtime.toml` by
...
fn default_executor_stack_size_bytes() -> usize {
    coop_app_host::host::DEFAULT_EXECUTOR_STACK_SIZE_BYTES
}

fn default_command_queue_capacity() -> usize {
    coop_app_host::host::DEFAULT_COMMAND_QUEUE_CAPACITY
}

fn default_gc_reclaim_check_interval() -> usize {
    coop_app_host::host::DEFAULT_GC_RECLAIM_CHECK_INTERVAL
}

fn default_gc_reclaim_growth_bytes() -> u64 {
    coop_app_host::host::DEFAULT_GC_RECLAIM_GROWTH_BYTES
}
...
fn default_cgroup_root() -> PathBuf {
    PathBuf::from("/sys/fs/cgroup/coop")
}
...
/// that works for local development (`./var/coop/...`); production installs
/// typically override to `/var/lib/coop/...`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PathsConfig {
    ...
    /// Path to the coop-worker binary. If not set, we locate it by
    /// searching `$PATH` and the daemon binary's own directory.
    #[serde(default)]
    pub coop_worker_binary: Option<PathBuf>,
}

fn default_deployments_dir() -> PathBuf {
    PathBuf::from("var/coop/deployments")
}
fn default_compiled_dir() -> PathBuf {
    PathBuf::from("var/coop/compiled")
}
fn default_sockets_dir() -> PathBuf {
    PathBuf::from("var/coop/sockets")
}
fn default_storage_dir() -> PathBuf {
    PathBuf::from("var/coop/storage")
}
fn default_logs_dir() -> PathBuf {
    PathBuf::from("var/coop/logs")
}
fn default_state_db() -> PathBuf {
    PathBuf::from("var/coop/state.sqlite")
}
fn default_acme_cache_dir() -> PathBuf {
    PathBuf::from("var/coop/acme")
}
...
fn default_perry_runtime_library() -> PathBuf {
    PathBuf::from(if cfg!(target_os = "macos") {
        "var/coop/lib/libperry_runtime.dylib"
    } else {
        "var/coop/lib/libperry_runtime.so"
    })
}
fn default_perry_stdlib_library() -> PathBuf {
    PathBuf::from(if cfg!(target_os = "macos") {
        "var/coop/lib/libperry_stdlib.dylib"
    } else {
        "var/coop/lib/libperry_stdlib.so"
    })
}
...
// ============================================================================
// coop.toml — per-deployment configuration
// ============================================================================

/// Per-deployment manifest. Lives at
/// `<deployments_dir>/<name>/coop.toml`. The daemon reads this every
...
impl DeploymentConfig {
    /// Load `coop.toml` from a deployment directory. The file must be
    /// named `coop.toml` and live directly under `deployment_dir`.
    pub fn load(deployment_dir: &Path) -> Result<Self> {
        let toml_path = deployment_dir.join("coop.toml");
        if !toml_path.exists() {
            return Err(anyhow!("deployment {:?} has no coop.toml", deployment_dir));
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for (name, value) in [
            ...
            if value > coop_host_abi::MAX_FRAME_SIZE {
                return Err(anyhow!(
                    "{name} ({value}) exceeds the global ABI frame limit ({})",
                    coop_host_abi::MAX_FRAME_SIZE
                ));
            }
        }
        Ok(())
...
"#,
    ...
    fn validates_invocation_and_byte_limits() {
        ...
        cfg.limits.max_request_body_bytes = coop_host_abi::MAX_FRAME_SIZE + 1;
        assert!(cfg
            .validate()
            .unwrap_err()
            .to_string()
