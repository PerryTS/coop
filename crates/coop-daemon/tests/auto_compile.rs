...
//!
//! This is the real Coop experience: drop TypeScript into a deployment
...
"#;
    ...
    let cgroup_config = std::env::var("COOP_TEST_CGROUP_ROOT")
        .ok()
        .map(|root| {
            format!(
                "\n[execution.cgroup]\nmode = \"required\"\nroot = {}\n",
...
"#,
    ...
    let prebuild = tokio::process::Command::new(coop_daemon_binary())
        .arg("--config")
        .arg(&rt)
        .arg("build")
        .arg("greeter")
    ...
    assert!(
        !artifact_namespace
            .join(".coop-deployment-state.json")
            .exists(),
    ...
    let first_prebuild_manifest = coop_host_abi::AppLibraryManifest::load(&first_prebuild_library)
        .unwrap()
        .unwrap();
    std::fs::write(
    ...
    .unwrap();
    let static_prebuild = tokio::process::Command::new(coop_daemon_binary())
        .arg("--config")
        .arg(&rt)
        .arg("build")
        .arg("greeter")
    ...
    let static_manifest = coop_host_abi::AppLibraryManifest::load(&static_library)
        .unwrap()
        .unwrap();
    assert_eq!(
    ...
    let initial_health: serde_json::Value = client
        .get(format!("{}/_coop/admin/deployments/greeter/health", base))
        .send()
        .await
    ...
    let memory_without_auth = client
        .get(format!("{}/_coop/admin/deployments/greeter/memory", base))
        .send()
        .await
    ...
    let no_op_reload = client
        .post(format!("{}/_coop/admin/deployments/greeter/reload", base))
        .basic_auth("coop", Some("test-secret"))
    ...
    assert!(
        !dep_dir.join("coop_entry.ts").exists(),
        "generated multi-entry source must be removed after compile"
    );
    let app_manifest = dylib.with_extension("coop-lib.json");
    assert!(
        app_manifest.exists(),
        "Coop should have written an ABI manifest at {:?}",
        app_manifest
    );
    assert!(
        dylib
    ...
    let manifest = coop_host_abi::AppLibraryManifest::load(&dylib)
        .unwrap()
        .expect("app manifest");
    ...
    assert_eq!(manifest.handler_abi, coop_host_abi::HandlerAbi::Wrapped);
    ...
    let config_v2 = std::fs::read_to_string(dep_dir.join("coop.toml"))
        .unwrap()
        .replace("max_request_body_bytes = 3", "max_request_body_bytes = 5");
    std::fs::write(dep_dir.join("coop.toml"), &config_v2).unwrap();
    ...
    std::fs::write(dep_dir.join("coop.toml"), &failing_health_config).unwrap();
    ...
    let artifact_status: serde_json::Value = client
        .get(format!(
            "{}/_coop/admin/deployments/greeter/artifacts",
            base
        ))
        .send()
        .await
    ...
    let rollback = client
        .post(format!(
            "{}/_coop/admin/deployments/greeter/rollback/{}",
            base, first_package
        ))
        .basic_auth("coop", Some("test-secret"))
    ...
    let metrics = client
        ...
        .await
        .expect("metrics body");
    assert!(metrics.contains("coop_requests_total"));
    assert!(metrics.contains("deployment=\"greeter\""));
    assert!(metrics.contains("coop_deployments_total 1"));
    assert!(metrics.contains("coop_invocation_rejections_total"));
    assert!(metrics.contains("reason=\"request_body\""));
    assert!(metrics.contains("coop_deployment_rollbacks_total"));
    assert!(metrics.contains("coop_deployment_activations_total"));
    assert!(metrics.contains("outcome=\"failure\""));
    assert!(metrics.contains("coop_artifact_retained_bytes"));
    assert!(metrics.contains("coop_application_arena_live_bytes"));
    assert!(metrics.contains("coop_application_arena_reserved_bytes"));
    assert!(metrics.contains("coop_deployment_process_isolated"));
    assert!(metrics.contains("coop_deployment_isolation_inherited"));
  ...
  if (requestPath === "/hang") { while (true) {} }"#,
        ...
        std::fs::write(dep_dir.join("coop.toml"), &dedicated_config).unwrap();
        ...
        if std::env::var_os("COOP_TEST_CGROUP_ROOT").is_some() {
        ...
        let restart_metrics = client
            ...
            .await
            .unwrap();
        assert!(restart_metrics.contains("coop_worker_restarts_total"));
        assert!(restart_metrics.contains("reason=\"exit\""));
        assert!(restart_metrics.contains("coop_worker_transport_backlog"));
        assert!(restart_metrics.contains("coop_worker_transport_in_flight"));
        assert!(restart_metrics.contains("coop_worker_transport_queue_wait_seconds"));
        assert!(restart_metrics.contains("coop_worker_transport_round_trip_seconds"));
        assert!(restart_metrics.contains("coop_worker_transport_bytes_total"));
        assert!(restart_metrics.contains("coop_worker_transport_poisoned_total"));
        ...
        let sidekick_config = config_v2
            ...
            + "\n\n[isolation]\nclass = \"sharded\"\n";
        std::fs::write(sidekick_dir.join("coop.toml"), &sidekick_config).unwrap();

        let sharded_config =
            dedicated_config.replace("class = \"dedicated\"", "class = \"sharded\"");
        std::fs::write(dep_dir.join("coop.toml"), &sharded_config).unwrap();
        ...
        let sidekick_reload = client
            .post(format!("{}/_coop/admin/deployments/sidekick/reload", base))
            .timeout(Duration::from_secs(30))
            .basic_auth("coop", Some("test-secret"))
            .header("x-coop-confirm", "reload")
            .send()
        ...
        let sidekick_shard: serde_json::Value = client
            .get(format!("{}/_coop/admin/deployments/sidekick/memory", base))
            .basic_auth("coop", Some("test-secret"))
            .send()
            .await
            .unwrap()
        ...
        std::fs::write(dep_dir.join("coop.toml"), trusted_config).unwrap();
        ...
        std::fs::write(sidekick_dir.join("coop.toml"), sidekick_trusted_config).unwrap();
        ...
        eprintln!("SKIP: coop-worker binary missing; isolation override roundtrip not run");
    }
    ...
    coop_app_host::initialize_runtime_libraries(&perry_runtime, &perry_stdlib)
        .expect("load providers for entry roundtrip");
    let host = coop_app_host::host::DeploymentHost::load("greeter", &dylib, None)
        .expect("load compiled multi-entry app");
    host.fire_cron(coop_host_abi::CronContext {
        expression: "*/1 * * * * *".into(),
        scheduled_at_ms: 42,
        dispatched_at_ms: 47,
    })
    ...
    let disposition = host
        .deliver_queue(coop_host_abi::QueueDispatchMessage {
    ...
    assert_eq!(disposition, coop_host_abi::QueueDisposition::Ack);
