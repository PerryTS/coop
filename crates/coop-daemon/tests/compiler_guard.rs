...
fn daemon_sigkill_cannot_orphan_guarded_compiler_processes() {
    ...
    let coop = env!("CARGO_BIN_EXE_coop");
    ...
    let script = r#"
        "$COOP_TEST_BIN" compiler-guard --parent-pid "$$" -- \
            sh -c 'echo $$ > "$COOP_TEST_PAYLOAD_PID"; exec sleep 60' &
        echo $! > "$COOP_TEST_GUARD_PID"
        wait
    "#;
    let parent = Command::new("sh")
        .arg("-c")
        .arg(script)
        .env("COOP_TEST_BIN", coop)
        .env("COOP_TEST_GUARD_PID", &guard_pid_file)
        .env("COOP_TEST_PAYLOAD_PID", &payload_pid_file)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .process_group(0)
