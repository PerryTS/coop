...
pub fn record_request(deployment: &str, method: &str, status: u16, duration_secs: f64) {
    ...
    let handles = RequestMetricHandles {
        requests: metrics::counter!("perch_requests_total", &labels),
        duration: metrics::histogram!("perch_request_duration_seconds", &labels),
    };
    let mut cache = cache
        .write()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
pub fn set_deployment_isolation(deployment: &str, process_isolated: bool, inherited: bool) {
    metrics::gauge!(
        "perch_deployment_process_isolated",
        "deployment" => deployment.to_string()
    )
    .set(if process_isolated { 1.0 } else { 0.0 });
    metrics::gauge!(
        "perch_deployment_isolation_inherited",
        "deployment" => deployment.to_string()
    )
...
pub fn set_deployment_shard(deployment: &str, shard: Option<(usize, u64)>) {
    metrics::gauge!(
        "perch_deployment_sharded",
        "deployment" => deployment.to_string()
    )
    .set(if shard.is_some() { 1.0 } else { 0.0 });
    metrics::gauge!(
        "perch_deployment_shard_slot",
        "deployment" => deployment.to_string()
    )
    .set(shard.map_or(-1.0, |(slot, _)| slot as f64));
    metrics::gauge!(
        "perch_deployment_shard_generation",
        "deployment" => deployment.to_string()
    )
...
pub fn record_shard_start(slot: usize, outcome: &str) {
    metrics::counter!(
        "perch_worker_shard_starts_total",
        "slot" => slot.to_string(),
...
pub fn record_shard_failure(slot: usize, reason: &str) {
    metrics::counter!(
        "perch_worker_shard_failures_total",
        "slot" => slot.to_string(),
...
pub fn set_shard_resident_deployments(slot: usize, count: usize) {
    metrics::gauge!(
        "perch_worker_shard_resident_deployments",
        "slot" => slot.to_string()
    )
    .set(count as f64);
}
...
pub fn record_worker_cgroup(deployment: &str, outcome: &str) {
    metrics::counter!(
        "perch_worker_cgroup_preparations_total",
        "deployment" => deployment.to_string(),
        "outcome" => outcome.to_string()
    )
    .increment(1);
...
pub fn set_worker_cgroup_stats(deployment: &str, stats: &crate::cgroup::WorkerCgroupStats) {
    for (name, value) in [
        (
            "perch_worker_cgroup_memory_current_bytes",
            stats.memory_current_bytes,
        ),
        (
            "perch_worker_cgroup_memory_peak_bytes",
            stats.memory_peak_bytes,
        ),
        ("perch_worker_cgroup_pids_current", stats.pids_current),
        ("perch_worker_cgroup_cpu_usage_usec", stats.cpu_usage_usec),
        (
            "perch_worker_cgroup_memory_oom_events",
            stats.memory_oom_events,
        ),
        (
            "perch_worker_cgroup_memory_oom_kill_events",
...
pub fn set_worker_restart_backoff(deployment: &str, seconds: f64) {
    metrics::gauge!(
        "perch_worker_restart_backoff_seconds",
        "deployment" => deployment.to_string()
    )
    .set(seconds);
}
...
pub fn set_worker_transport(deployment: &str, backlog: usize, in_flight: usize) {
    metrics::gauge!(
        "perch_worker_transport_backlog",
        "deployment" => deployment.to_string()
    )
    .set(backlog as f64);
    metrics::gauge!(
        "perch_worker_transport_in_flight",
        "deployment" => deployment.to_string()
    )
...
pub fn record_worker_transport_queue_wait(
    ...
    metrics::histogram!(
        "perch_worker_transport_queue_wait_seconds",
        "deployment" => deployment.to_string(),
...
pub fn record_worker_transport_round_trip(
    ...
    metrics::histogram!(
        "perch_worker_transport_round_trip_seconds",
        "deployment" => deployment.to_string(),
        "entrypoint" => entrypoint.to_string(),
        "outcome" => outcome.to_string()
    )
...
pub fn record_worker_transport_bytes(
    ...
    metrics::counter!(
        "perch_worker_transport_bytes_total",
        "deployment" => deployment.to_string(),
        "entrypoint" => entrypoint.to_string(),
...
pub fn record_worker_transport_cancellation(deployment: &str, entrypoint: &str, phase: &str) {
    metrics::counter!(
        "perch_worker_transport_cancellations_total",
        "deployment" => deployment.to_string(),
        "entrypoint" => entrypoint.to_string(),
...
pub fn record_worker_transport_poisoned(deployment: &str, cause: &str) {
    metrics::counter!(
        "perch_worker_transport_poisoned_total",
        "deployment" => deployment.to_string(),
...
pub fn record_worker_transport_drain(deployment: &str, outcome: &str, duration_secs: f64) {
    metrics::histogram!(
        "perch_worker_transport_drain_duration_seconds",
        "deployment" => deployment.to_string(),
        "outcome" => outcome.to_string()
    )
    .record(duration_secs);
...
pub fn record_invocation_rejected(deployment: &str, entrypoint: &str, reason: &str) {
    metrics::counter!(
        "perch_invocation_rejections_total",
        "deployment" => deployment.to_string(),
        "entrypoint" => entrypoint.to_string(),
        "reason" => reason.to_string()
    )
...
pub fn record_invocation_timeout(deployment: &str, entrypoint: &str) {
    metrics::counter!(
        "perch_invocation_timeouts_total",
        "deployment" => deployment.to_string(),
        "entrypoint" => entrypoint.to_string()
    )
    .increment(1);
...
pub fn set_application_arena(deployment: &str, live_bytes: u64, reserved_bytes: u64) {
    metrics::gauge!(
        "perch_application_arena_live_bytes",
        "deployment" => deployment.to_string()
    )
    .set(live_bytes as f64);
    metrics::gauge!(
        "perch_application_arena_reserved_bytes",
        "deployment" => deployment.to_string()
    )
...
pub fn set_artifact_inventory(deployment: &str, packages: usize, retained_bytes: u64) {
    metrics::gauge!(
        "perch_artifact_packages",
        "deployment" => deployment.to_string()
    )
    .set(packages as f64);
    metrics::gauge!(
        "perch_artifact_retained_bytes",
        "deployment" => deployment.to_string()
    )
...
pub fn record_artifact_collection(deployment: &str, outcome: &str, removed: usize) {
    metrics::counter!(
        "perch_artifact_collections_total",
        "deployment" => deployment.to_string(),
        "outcome" => outcome.to_string()
    )
    .increment(1);
    if removed > 0 {
        metrics::counter!(
            "perch_artifact_packages_collected_total",
            "deployment" => deployment.to_string()
        )
...
pub fn record_artifact_reconciliation(outcome: &str, removed: usize) {
    metrics::counter!(
        "perch_artifact_reconciliations_total",
        "outcome" => outcome.to_string()
    )
    .increment(1);
    if removed > 0 {
        metrics::counter!("perch_artifact_entries_reconciled_total").increment(removed as u64);
    }
}

pub fn record_rollback(deployment: &str, outcome: &str) {
    metrics::counter!(
        "perch_deployment_rollbacks_total",
        "deployment" => deployment.to_string(),
        "outcome" => outcome.to_string()
    )
    .increment(1);
...
pub fn record_activation(deployment: &str, outcome: &str, duration_secs: f64, requests: u16) {
    ...
    metrics::counter!("perch_deployment_activations_total", &labels).increment(1);
    metrics::histogram!("perch_deployment_activation_duration_seconds", &labels)
        .record(duration_secs);
    metrics::histogram!("perch_deployment_activation_requests", &labels).record(requests as f64);
    metrics::gauge!(
        "perch_deployment_activation_healthy",
        "deployment" => deployment.to_string()
    )
...
pub fn record_runtime_reuse(deployment: &str) {
    metrics::counter!(
        "perch_deployment_runtime_reuses_total",
        "deployment" => deployment.to_string()
    )
    .increment(1);
}
...
pub fn record_compile_queue_wait(deployment: &str, duration_secs: f64) {
    metrics::histogram!(
        "perch_compile_queue_wait_seconds",
        "deployment" => deployment.to_string()
    )
    .record(duration_secs);
}
...
pub fn record_compile(
    ...
    metrics::counter!("perch_compiles_total", &labels).increment(1);
    metrics::histogram!("perch_compile_duration_seconds", &labels).record(duration_secs);
    if peak_rss_bytes > 0 {
        metrics::histogram!("perch_compile_peak_rss_bytes", &labels).record(peak_rss_bytes as f64);
    }
}
...
pub fn record_compile_phase(deployment: &str, phase: &str, outcome: &str, duration_secs: f64) {
    ...
    metrics::counter!("perch_compile_phases_total", &labels).increment(1);
    metrics::histogram!("perch_compile_phase_duration_seconds", &labels).record(duration_secs);
}

pub fn record_compile_cache_entries(deployment: &str, layer: &str, outcome: &str, entries: u64) {
    metrics::counter!(
        "perch_compile_cache_entries_total",
        "deployment" => deployment.to_string(),
...
pub fn set_queue_stats(
    ...
    metrics::gauge!("perch_queue_depth", &labels).set(total_depth as f64);
    metrics::gauge!("perch_queue_visible_depth", &labels).set(visible_depth as f64);
    metrics::gauge!("perch_queue_active_leases", &labels).set(active_leases as f64);
    metrics::gauge!("perch_queue_oldest_visible_age_seconds", &labels)
        .set(oldest_visible_age_seconds);
    metrics::gauge!("perch_queue_dead_letters", &labels).set(dead_letters as f64);
}

pub fn record_queue_claim(deployment: &str, queue: &str, recovered_expired_lease: bool) {
    metrics::counter!(
        "perch_queue_claims_total",
        "deployment" => deployment.to_string(),
        "queue" => queue.to_string(),
    ...
    if recovered_expired_lease {
        metrics::counter!(
            "perch_queue_expired_leases_total",
            "deployment" => deployment.to_string(),
            "queue" => queue.to_string()
        )
        .increment(1);
...
pub fn record_cron_schedule(deployment: &str, schedule: &str, outcome: &str) {
    metrics::counter!(
        "perch_cron_schedule_events_total",
        "deployment" => deployment.to_string(),
...
pub fn record_cron_completion(deployment: &str, schedule: &str, outcome: &str, duration_secs: f64) {
    ...
    metrics::counter!("perch_cron_invocations_total", &labels).increment(1);
    metrics::histogram!("perch_cron_duration_seconds", &labels).record(duration_secs);
}
...
pub fn record_queue_delivery(deployment: &str, queue: &str, outcome: &str, duration_secs: f64) {
    ...
    metrics::counter!("perch_queue_deliveries_total", &labels).increment(1);
    metrics::histogram!("perch_queue_delivery_duration_seconds", &labels).record(duration_secs);
}

pub fn record_queue_retry(deployment: &str, queue: &str, outcome: &str) {
    metrics::counter!(
        "perch_queue_retries_total",
        "deployment" => deployment.to_string(),
        "queue" => queue.to_string(),
        "outcome" => outcome.to_string()
    )
...
pub fn record_queue_operator_action(deployment: &str, queue: &str, action: &str, outcome: &str) {
    metrics::counter!(
        "perch_queue_operator_actions_total",
        "deployment" => deployment.to_string(),
        "queue" => queue.to_string(),
...
pub fn record_queue_store_error(operation: &str) {
    metrics::counter!(
        "perch_queue_store_errors_total",
        "operation" => operation.to_string()
    )
    .increment(1);
}

pub fn set_queue_pool_status(status: &perch_app_host::queue_store::QueuePoolStatus) {
    metrics::gauge!("perch_queue_pool_connections", "state" => "max").set(status.max_size as f64);
    metrics::gauge!("perch_queue_pool_connections", "state" => "open").set(status.size as f64);
    metrics::gauge!("perch_queue_pool_connections", "state" => "available")
        .set(status.available as f64);
    metrics::gauge!("perch_queue_pool_waiters").set(status.waiting as f64);
    ...
    metrics::gauge!("perch_queue_pool_utilization_ratio").set(utilization);
}

pub fn record_queue_dlq_pruned(deployment: &str, queue: &str, removed: u64) {
    if removed > 0 {
        metrics::counter!(
            "perch_queue_dead_letters_pruned_total",
            "deployment" => deployment.to_string(),
            "queue" => queue.to_string()
        )
        .increment(removed);
...
mod tests {
    ...
    fn request_metric_hot_path_cost() {
        ...
        let max_nanoseconds = std::env::var("PERCH_METRICS_MAX_NS_PER_REQUEST")
            .ok()
