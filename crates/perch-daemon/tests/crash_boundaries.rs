...
fn perry_libraries() -> (PathBuf, PathBuf) {
    ...
    let root = workspace_root().join("var/perch/lib");
    (
...
fn spawn_daemon(config: &Path, crash_point: Option<(&str, &Path, &Path)>) -> tokio::process::Child {
    let mut command = tokio::process::Command::new(env!("CARGO_BIN_EXE_perch"));
    command
        .arg("--config")
        .arg(config)
        .stdin(Stdio::null())
        .kill_on_drop(true);
    if std::env::var_os("PERCH_TEST_VERBOSE_CRASH").is_some() {
    ...
    if let Some((point, marker, arm)) = crash_point {
        command
            .env("PERCH_TEST_PROCESS_CRASH_POINT", point)
            .env("PERCH_TEST_PROCESS_CRASH_DEPLOYMENT", "crash-app")
            .env("PERCH_TEST_PROCESS_CRASH_MARKER", marker)
            .env("PERCH_TEST_PROCESS_CRASH_ARM", arm);
    }
    command.spawn().expect("spawn Perch daemon")
}
...
"#,
            ...
            workspace_root().join("target/debug/perch-worker").display(),
            perry.display(),
            runtime.display(),
            stdlib.display(),
            password_hash,
    ...
    for (index, point) in points.into_iter().enumerate() {
        ...
        let reload_url = format!("{base}/_perch/admin/deployments/crash-app/reload");
        ...
        let response = client
            .post(format!("{base}/_perch/admin/deployments/crash-app/reload"))
            .basic_auth("perch", Some("test-secret"))
            .header("x-perch-confirm", "reload")
            .send()
            .await
