...
//! included in daemon startup. Each scenario starts a real Perch daemon,
...
async fn measure_in_process_startup_and_rss() {
    ...
    let daemon = std::env::var_os("PERCH_BENCH_DAEMON")
        .map(PathBuf::from)
        .unwrap_or_else(|| workspace.join("target/release/perch"));
    ...
    let runtime = workspace.join(format!(
        "var/perch/lib/libperry_runtime.{library_extension}"
    ));
    let stdlib = workspace.join(format!("var/perch/lib/libperry_stdlib.{library_extension}"));
    ...
    let requests = std::env::var("PERCH_BENCH_REQUESTS")
        ...
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(0);
    let preload_concurrency = std::env::var("PERCH_BENCH_PRELOAD_CONCURRENCY")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(4);
    ...
    let scenarios = std::env::var("PERCH_BENCH_APP_COUNTS")
        .ok()
        .map(|value| {
            value
                .split(',')
    ...
    let fixture_parent = workspace.join("target/perch-resource-benchmark");
...
fn prepare_fixture(
    ...
    let source_config: serde_json::Value = serde_json::from_slice(
        &std::fs::read(source_package.join("deployment.perch.json"))
            .expect("prepared app has packaged deployment config"),
    )
    .expect("parse prepared deployment config");
    let static_manifest_bytes = std::fs::read(source_package.join("static.perch-manifest.json"))
...
"#,
        ...
        if manifest.boundary_verified {
            ...
            manifest.library_sha256 = Some(
                perch_app_host::plugin_host::library_sha256(source_app)
        ...
        std::fs::write(package_dir.join("deployment.perch.json"), config_bytes)
            .expect("write packaged deployment config");
        std::fs::write(
            package_dir.join("static.perch-manifest.json"),
        ...
        std::fs::write(
            compiled.join(&name).join(".perch-deployment-state.json"),
...
"#,
            ...
            workspace
                .join(format!(
                    "var/perch/lib/libperry_runtime.{}",
                    shared_library_extension()
                ))
                .display(),
            workspace
                .join(format!(
                    "var/perch/lib/libperry_stdlib.{}",
                    shared_library_extension()
                ))
                .display(),
        ),
...
fn start_and_wait(
    ...
    let trace_startup = std::env::var_os("PERCH_BENCH_TRACE_STARTUP").is_some();
