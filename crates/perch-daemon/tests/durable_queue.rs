...
fn spawn(config: &Path) -> tokio::process::Child {
    tokio::process::Command::new(root().join("target/debug/perch"))
        .args(["--config"])
        .arg(config)
        .env("RUST_LOG", "warn")
...
async fn wait_for_listener(
    ...
    for _ in 0..180 {
        ...
        if client
            .get(format!(
                "{base_url}/_perch/admin/deployments/queue-app/artifacts"
            ))
            .send()
            .await
...
async fn wait_for_queue_empty(pg: &tokio_postgres::Client) {
    for _ in 0..250 {
        let pending: i64 = pg
            .query_one(
                "SELECT count(*)::bigint FROM perch_queue_messages WHERE deployment = 'queue-app'",
                &[],
            )
            .await
            .unwrap()
...
impl PostgresContainerOutage {
    fn stop(name: String) -> Self {
        assert!(
            ...
            "PERCH_TEST_POSTGRES_CONTAINER contains unsafe characters"
        );
...
fn assert_stable_application_exports(compiled: &Path) {
    ...
    assert!(symbols.contains("perch_app_http_v2"));
    assert!(symbols.contains("perch_app_queue_0_v2"));
    assert!(!symbols.contains("__perry_wrap_perry_fn_"));
    let manifest = perch_host_abi::AppLibraryManifest::load(&library)
        .unwrap()
        .unwrap();
    assert_eq!(manifest.handle_symbol, "perch_app_http_v2");
    assert_eq!(manifest.queue_entries[0].symbol, "perch_app_queue_0_v2");
}
...
#[tokio::test]
#[ignore = "requires PERCH_TEST_POSTGRES_URL and built Perry providers"]
async fn runtime_send_is_tenant_bound_durable_and_consumed() {
    let postgres_url = std::env::var("PERCH_TEST_POSTGRES_URL")
        .expect("PERCH_TEST_POSTGRES_URL must point to a disposable database");
    ...
    std::fs::create_dir_all(deployment.join("node_modules/@perch")).unwrap();
    #[cfg(unix)]
    std::os::unix::fs::symlink(
        root().join("packages/perch-runtime"),
        deployment.join("node_modules/@perch/runtime"),
    )
    .unwrap();
    std::fs::write(
  ...
  "name": "queue-app",
  "private": true,
  "perry": { "allow": { "nativeLibrary": ["@perch/runtime/src/queue"] } }
}"#,
    )
    .unwrap();
    std::fs::write(
        deployment.join("perch.toml"),
        r#"
name = "queue-app"
[hosts]
domains = ["queue.test"]
...
"#,
    ...
    std::fs::write(
        ...
        r#"
import { queue } from "@perch/runtime/src/queue";
...
"#,
    ...
    let worker = root().join("target/debug/perch-worker");
    assert!(
        worker.exists(),
        "build perch-worker before this ignored test"
    );
...
"#,
    ...
    let metrics = client
        ...
        .await
        .unwrap();
    assert!(metrics.contains("perch_queue_claims_total"));
    assert!(metrics.contains("perch_queue_deliveries_total"));
    assert!(metrics.contains("perch_queue_pool_connections"));
    assert!(metrics.contains("perch_queue_pool_waiters"));
    assert!(metrics.contains("perch_queue_pool_utilization_ratio"));
    ...
    let metrics = client
        .get(format!("{worker_url}/_perch/metrics"))
        .send()
        .await
        .unwrap()
        .text()
        .await
        .unwrap();
    assert!(metrics.contains("perch_queue_retries_total"));
    assert!(metrics.contains("outcome=\"scheduled\""));
    assert!(metrics.contains("outcome=\"exhausted_dlq\""));

    let dlq_url =
        format!("{worker_url}/_perch/admin/deployments/queue-app/queues/jobs/dead-letters");
    assert_eq!(
...
"#,
    ...
    let replay = client
        ...
        .basic_auth("perch", Some("test-secret"))
        .header("x-perch-confirm", "replay-dead-letter")
        .send()
        .await
        .unwrap();
    ...
    let replay_source: i64 = pg
        .query_one(
            "SELECT count(*)::bigint FROM perch_queue_dead_letters WHERE id = $1",
            &[&operator_replay_id],
        )
        .await
        .unwrap()
    ...
    let purge = client
        .delete(format!("{dlq_url}/{exhausted_dlq_id}"))
        .basic_auth("perch", Some("test-secret"))
        .header("x-perch-confirm", "purge-dead-letter")
        .send()
        .await
        .unwrap();
...
"#,
    ...
    client
        .post(format!(
            "{worker_url}/_perch/admin/deployments/queue-app/reload"
        ))
        .basic_auth("perch", Some("test-secret"))
    ...
    client
        .post(format!(
            "{worker_url}/_perch/admin/deployments/queue-app/rollback/{old_package}"
        ))
        .basic_auth("perch", Some("test-secret"))
    ...
    let memory = client
        .get(format!(
            "{shard_url}/_perch/admin/deployments/queue-app/memory"
        ))
        .basic_auth("perch", Some("test-secret"))
        .send()
        .await
    ...
    wait_for_queue_empty(&pg).await;
    let metrics = client
        .get(format!("{shard_url}/_perch/metrics"))
        .send()
        .await
        .unwrap()
        .text()
    ...
    assert!(metrics.contains("perch_queue_deliveries_total"));
    assert!(metrics.contains("outcome=\"ack\""));
    assert!(metrics.contains("perch_worker_shard_resident_deployments"));
    stop_daemon(&mut daemon).await;
    ...
    wait_for_queue_empty(&pg).await;
    let metrics = client
        .get(format!("{recovery_url}/_perch/metrics"))
        .send()
        .await
        .unwrap()
        .text()
        .await
        .unwrap();
    assert!(metrics.contains("perch_queue_expired_leases_total"));
...
"#,
    ...
    let pg = if let Ok(container) = std::env::var("PERCH_TEST_POSTGRES_CONTAINER") {
