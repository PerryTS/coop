//! coop-cli — developer CLI for Coop.
//!
//! Subcommands:
//! - `coop-cli status` — show deployment list from the daemon admin API
//! - `coop-cli deploy <dir> <target>` — rsync + trigger reload
//! - `coop-cli logs <deployment>` — tail deployment logs (placeholder)
//! - `coop-cli rollback <deployment>` — rollback (placeholder)
//! - `coop-cli dev <dir>` — local dev mode (placeholder)

use clap::{Parser, Subcommand};

#[derive(Debug, Parser)]
#[command(name = "coop-cli", about = "Coop developer CLI", version)]
struct Cli {
    #[command(subcommand)]
    command: Command,

    /// Base URL of the coop daemon. Defaults to http://127.0.0.1:80.
    #[arg(long, env = "COOP_URL", default_value = "http://127.0.0.1:80")]
    url: String,
}
...
enum Command {
    ...
    /// Deploy a directory to the coop box.
    Deploy {
        /// Local directory containing the deployment (with coop.toml).
        dir: String,
        /// Remote target (e.g., root@box:/var/lib/coop/deployments/name).
        target: String,
    },
...
async fn cmd_status(base_url: &str) -> anyhow::Result<()> {
    ...
    let url = format!("{}/_coop/admin", base_url.trim_end_matches('/'));
        ...
        Err(e) => {
            anyhow::bail!(
                "Could not connect to daemon at {}: {}. \
                 Is `coop` running? Set --url or COOP_URL if the \
...
async fn cmd_logs(_base_url: &str, deployment: &str) -> anyhow::Result<()> {
    println!(
        "Log tailing for '{}' is not yet implemented. \
         Check /_coop/admin for recent logs.",
        deployment
    );
    Ok(())
}
...
async fn cmd_dev(dir: &str) -> anyhow::Result<()> {
    println!(
        "Dev mode for '{}' is not yet implemented. \
         Run `coop --config ./dev-runtime.toml` with a local \
